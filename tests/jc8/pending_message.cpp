...
void init_Context(std::shared_ptr<Context>& ctx)
{
  ...
  ctx->identity = std::make_shared<ContextIdentity>();
  ctx->identity->app_install_id = "app_install_id";
  ctx->identity->advertising_id = "advertising_id";
  ctx->identity->device_id = "device_id";
}
